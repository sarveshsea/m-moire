...
import { AbstractJsonLineDriver, defaultCodexShapedEmit } from "../json-line-driver.js";
class FixtureDriver extends AbstractJsonLineDriver {
  protected binding() { return { discriminatorField: "kind" as const, defaultModel: "fixture-model", processName: "fixture", emit: defaultCodexShapedEmit }; }
}
...
function fixture(options: Record<string, unknown> = {}) {
  ...
  const driver = new FixtureDriver(config);
...
describe("json-line-driver release protocol boundaries", () => {
  ...
  it.each(["__proto__", "constructor", "toString"])("treats inherited event tag %s as unknown data", async (tag) => {
    const f = fixture(); await run(f.driver.start());
    try {
      expect(() => f.frame(tag)).not.toThrow();
      expect(f.events.at(-1)).toMatchObject({ type: "diagnostic.warn", message: expect.stringContaining(tag) });
    } finally { await run(f.driver.shutdown()); }
  });
  it.each([undefined, {}, { spawnTransport: "invalid" }])("rejects invalid transport config %j", async options => {
    const f = fixture();
    const driver = new FixtureDriver({ ...f.driver.config, options });
  ...
  it("forwards explicit spawn settings and uses defaults when absent", async () => {
    for (const options of [{}, { model: "local-model", env: { FIXTURE: "yes" }, cwd: "/fixture", baseUrl: "http://127.0.0.1:12345" }]) {
      ...
      expect(f.spawnTransport).toHaveBeenCalledWith(expect.objectContaining({ model: options.model ?? "fixture-model", env: options.env ?? process.env, cwd: options.cwd ?? process.cwd() }));
  ...
  it("handles a failed turn with no error detail and provider-default completion", async () => {
    ...
    f.frame("turn_completed"); expect(f.events.at(-1)).toMatchObject({ type: "turn.completed", outcome: "success" });
    await run(f.driver.shutdown());
  });
