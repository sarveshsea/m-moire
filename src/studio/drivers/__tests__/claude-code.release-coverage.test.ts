...
function fixture(options: Record<string, unknown> = {}) {
  ...
  const driver = new ClaudeCodeDriver(config);
  ...
  return { driver, events, transport, spawnTransport, lines, exits, line,
    frame: (tag: string, value: Record<string, unknown> = {}, stream: "stdout" | "stderr" = "stdout") => line(JSON.stringify({ type: tag, ...value }), stream),
...
describe("claude-code release protocol boundaries", () => {
  ...
  it.each([undefined, {}, { spawnTransport: "invalid" }])("rejects invalid transport config %j", async options => {
    const f = fixture();
    const driver = new ClaudeCodeDriver({ ...f.driver.config, options });
  ...
  it("forwards explicit spawn settings and uses defaults when absent", async () => {
    for (const options of [{}, { model: "local-model", env: { FIXTURE: "yes" }, cwd: "/fixture", baseUrl: "http://127.0.0.1:12345" }]) {
      ...
      expect(f.spawnTransport).toHaveBeenCalledWith(expect.objectContaining({ model: options.model ?? "claude-sonnet-4-6", env: options.env ?? process.env, cwd: options.cwd ?? process.cwd() }));
  ...
  it("writes the full prompt while bounding the turn preview and preserves ordered event envelopes", async () => {
    ...
    expect(JSON.parse(f.transport.write.mock.calls[0][0])).toEqual({ type: "user_turn", prompt });
  ...
  it.each([undefined, "operator cancellation"])("interrupts the active turn with reason %s", async reason => {
    ...
    expect(JSON.parse(f.transport.write.mock.calls[1][0])).toEqual({ type: "interrupt", reason: reason ?? "user" });
  ...
  it("converts message, tool and usage payloads to the canonical event envelope", async () => {
    const f = fixture(); await run(f.driver.start()); await run(f.driver.sendTurn(turn));
    f.frame("assistant_delta", { text: "Hello " });
    f.frame("assistant_message", { text: "Hello world" });
    f.frame("tool_use_start", { toolCallId: "tcl_fixture", tool: "Read", input: { path: "Button.tsx" } });
    f.frame("tool_use_output", { toolCallId: "tcl_fixture", chunk: "source", stream: "stderr" });
    f.frame("tool_use_complete", { toolCallId: "tcl_fixture", ok: true, elapsedMs: "12", result: { found: true } });
  ...
  it("provides defined defaults for optional protocol fields", async () => {
    ...
    f.frame("tool_use_start"); expect(f.events.at(-1)).toMatchObject({ tool: "unknown", toolCallId: expect.any(String) });
    f.frame("tool_use_output", { toolCallId: "tcl_defaults" }); expect(f.events.at(-1)).toMatchObject({ toolCallId: "tcl_defaults", chunk: "", stream: "stdout" });
    f.frame("tool_use_complete", { toolCallId: "tcl_defaults" }); expect(f.events.at(-1)).toMatchObject({ toolCallId: "tcl_defaults", ok: false, elapsedMs: 0 });
  ...
  it.each([true, false])("completes an active turn ok=%s and clears its identity", async ok => {
    const f = fixture(); await run(f.driver.start()); await run(f.driver.sendTurn(turn));
    f.frame("turn_complete", { ok, error: "fixture error" });
    ...
    f.frame("turn_complete", { ok: false });
  ...
  it("handles a failed turn with no error detail and provider-default completion", async () => {
    ...
    f.frame("turn_complete"); expect(f.events.at(-1)).toMatchObject({ type: "turn.completed", outcome: "success" });
    await run(f.driver.shutdown());
  });

  it("maps explicit and default MCP status and approval requests", async () => {
    const f = fixture(); await run(f.driver.start());
    f.frame("mcp_status", { serverName: "memoire", status: "ready", message: "local" });
    expect(f.events.at(-1)).toMatchObject({ type: "mcp.status.updated", serverName: "memoire", status: "ready", message: "local" });
    f.frame("mcp_status"); expect(f.events.at(-1)).toMatchObject({ serverName: "", status: "connecting" });
    f.frame("approval_request", { approvalId: "approval1", tool: "Write", args: { path: "Button.tsx" }, reason: "edit requested" });
    expect(f.events.at(-1)).toMatchObject({ type: "approval.requested", approvalId: "approval1", tool: "Write", reason: "edit requested", args: { path: "Button.tsx" } });
    f.frame("approval_request"); expect(f.events.at(-1)).toMatchObject({ approvalId: expect.any(String), tool: "unknown", reason: "" });
    await run(f.driver.shutdown());
  });

  it.each(["tool_use_output", "tool_use_complete"])("diagnoses missing tool identity in %s and continues parsing", async tag => {
    const f = fixture(); await run(f.driver.start());
    try {
  ...
  it("keeps malformed field diagnostics bounded and excludes raw error details", async () => {
    const f = fixture(); await run(f.driver.start());
    try {
      expect(() => f.frame("tool_use_output", { toolCallId: "PRIVATE_INVALID_ID" }, "stderr")).not.toThrow();
      ...
      expect(() => f.line('{"type":{"toString":null}}', "stderr")).not.toThrow();
