...
function fixture(options: Record<string, unknown> = {}) {
  ...
  const driver = new CodexDriver(config);
...
describe("codex release protocol boundaries", () => {
  ...
  it.each([undefined, {}, { spawnTransport: "invalid" }])("rejects invalid transport config %j", async options => {
    const f = fixture();
    const driver = new CodexDriver({ ...f.driver.config, options });
  ...
  it("forwards explicit spawn settings and uses defaults when absent", async () => {
    for (const options of [{}, { model: "local-model", env: { FIXTURE: "yes" }, cwd: "/fixture", effort: "low" }]) {
      const f = fixture(options); await run(f.driver.start());
      expect(f.spawnTransport).toHaveBeenCalledWith(expect.objectContaining({ model: options.model ?? "gpt-5.5", effort: options.effort ?? "xhigh", env: options.env ?? process.env, cwd: options.cwd ?? process.cwd() }));
