...
async function setup(){const root=await mkdtemp(join(tmpdir(),'memi-frontend-command-'));roots.push(root);await mkdir(join(root,'src'));await writeFile(join(root,'src/Button.tsx'),'export function Button({disabled}: {disabled?:boolean}) {return <button disabled={disabled}/>;}');const command=new Command().exitOverride();registerAgentCommand(command,{config:{projectRoot:root}} as MemoireEngine);const logs=vi.spyOn(console,'log').mockImplementation(()=>{});return {root,command,logs};}
