...
The preceding audit recorded 2,447 passing tests and a local review package of 620,548 bytes packed / 2,131,199 bytes unpacked / 61 files, with 50,948,192 bytes installed. These are candidate observations, not published artifact guarantees. Source-wide coverage remains 64.46% statements, 55.14% branches, 74.33% functions, and 66.05% lines; all four fail the 80% publication requirement. The green CI coverage job covers a narrower boundary. `prepublishOnly` correctly remains blocking.
...
1. Keep the current candidate unpublished while source-wide coverage, command compatibility, output semantics, and applicable beta evidence remain open. The managed scan may only be explicitly pending for beta under the existing release policy; it is required for stable. Preserve the existing beta parity exceptions as explicit exceptions, never passing receipts.
