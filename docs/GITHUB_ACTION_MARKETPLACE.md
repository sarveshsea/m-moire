...
| --- | --- | --- |
| `version` | `2.7.9` | Exact CLI version. The reviewed default is pinned; exact prior versions remain accepted for existing workflows. Ranges and dist-tags are rejected. |
...
4. Create the immutable `v2.7.9` release tag on that commit. Never retag an existing release.
